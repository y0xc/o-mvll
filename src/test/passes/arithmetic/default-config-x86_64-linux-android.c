...
// RUN: env OMVLL_CONFIG=%S/config_defaultConfig_include_seed.py         clang -target x86_64-pc-linux-gnu -fpass-plugin=%libOMVLL -fno-verbose-asm -O1 -S %s -o - | FileCheck --check-prefix=NEWSEED %s
